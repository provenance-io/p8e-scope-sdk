...
import objectstore.MailboxServiceGrpc
import objectstore.Mailbox
import objectstore.ObjectServiceGrpc
...
open class OsClient(
    ...
    fun ack(uuid: UUID) {
        val request = Mailbox.AckRequest.newBuilder()
    ...
    fun put(
        ...
        val responseObserver = SingleResponseObserver<Object.ObjectResponse>()
        val requestObserver = objectAsyncClient.put(responseObserver)
        val header = Object.MultiStreamHeader.newBuilder()
            .setStreamCount(1)
        ...
        dimeInputStream.use {
            try {
                requestObserver.onNext(Object.ChunkBidi.newBuilder().setMultiStreamHeader(header).build())
    ...
    fun createPublicKey(publicKey: PublicKey): ObjectStorePrivateKey.PublicKeyResponse? =
        publicKeyBlockingClient.withDeadlineAfter(deadlineMs, TimeUnit.MILLISECONDS)
            .add(
                ObjectStorePrivateKey.PublicKeyRequest.newBuilder()
    ...
    Object.ChunkBidi.newBuilder()
        .setChunk(
            Object.Chunk.newBuilder()
                .setHeader(
                    Object.StreamHeader.newBuilder()
                        .setName(pair.first)
                        .build()
                )
