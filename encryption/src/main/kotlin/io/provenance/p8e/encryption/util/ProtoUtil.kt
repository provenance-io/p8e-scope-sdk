...
import io.provenance.scope.encryption.Common.DefinitionSpec
import io.provenance.scope.encryption.Common.DefinitionSpec.Type
import io.provenance.scope.encryption.Common.Location
import io.provenance.scope.encryption.Common.OutputSpec
import io.provenance.scope.encryption.Common.ProvenanceReference
import io.provenance.scope.encryption.Common.Signature
import io.provenance.scope.encryption.Contracts.ProposedFact
import io.provenance.scope.encryption.Util
...
object ProtoUtil {
    ...
    fun provenanceReferenceOf(scopeUuid: UUID, contractUuid: UUID, hash: String) =
        ProvenanceReference.newBuilder()
            .setScopeUuid(
                Util.UUID.newBuilder().setValue(scopeUuid.toString()).build())
            .setGroupUuid(Util.UUID.newBuilder().setValue(contractUuid.toString()).build())
            .setHash(hash)
